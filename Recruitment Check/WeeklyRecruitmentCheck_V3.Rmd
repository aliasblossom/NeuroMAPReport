...
MASTER <- read_xlsx(path = "/Users/bbh5255/Box Sync/DEPENd/NeuroMAP/Recruitment/Participant Management/NEUROMAP Master V2.xlsx", sheet = "MasterData")
ENROLL_OVERVIEW <- read_xlsx("/Users/bbh5255/Box Sync/DEPENd/NeuroMAP/Recruitment/Participant Management/NEUROMAP Participant Tracker.xlsx", sheet = "RO1 Aggregate")
ENROLL_MASTER <- read_xlsx("/Users/bbh5255/Box Sync/DEPENd/NeuroMAP/Recruitment/Participant Management/NEUROMAP Participant Tracker.xlsx", sheet = "Master")
...
TO_DATE <- MASTER %>% 
  ...
  filter(SCREEN_SENTDATE >= as.Date("2019-07-01") & SCREEN_SENTDATE <= as.Date("2019-08-12")) # %>%
#group_by(STATUS) %>%
#tally()
...
UPCOMMING_ALL <- ENROLL_MASTER %>%
  select(NAME, ID, AGE, GENDER, GROUP, S1_DATE) %>%
  filter(S1_DATE >= as.Date("2019-08-13") & S1_DATE <= as.Date("2019-08-30")) %>%
  arrange(S1_DATE)

knitr::kable(
  UPCOMMING_ALL, 
  caption = "All Upcomming Sessions",
  col.names = c("Name", "ID", "Age", "Gender", "Group", "Date of Session 1"))
...
#MUST CHANGE DATE
UPCOMMING <- ENROLL_MASTER %>%
  select(NAME, ID, AGE, GENDER, GROUP, R01_ENROLLMENT, S1_DATE) %>%
  filter(S1_DATE >= as.Date("2019-08-12") & S1_DATE <= as.Date("2019-08-17")) 

knitr::kable(
  UPCOMMING, 
  caption = "Sessions Scheduled This Week",
  col.names = c("Name", "ID", "Age", "Gender", "Group", "fMRI Elegible", "Date of Session 1"))

#sessions last week
#MUST CHANGE DATE
LASTWEEK <- ENROLL_MASTER %>%
  select(NAME, ID, AGE, GENDER, GROUP, R01_ENROLLMENT, S1_DATE, S2_DATE) %>%
  filter(S1_DATE >= as.Date("2019-08-05") & S1_DATE <= as.Date("2019-08-12")) 

knitr::kable(
  LASTWEEK, 
  caption = "Sessions Scheduled Last Week",
  col.names = c("Name", "ID", "Age", "Gender", "Group", "fMRI Elegible", "Date of Session 1", "Date of Session 2"))
...
WEEKLY <- MASTER %>% 
  ...
  filter(SCREEN_SENTDATE >= as.Date("2019-08-05") & SCREEN_SENTDATE <= as.Date("2019-08-12"))
