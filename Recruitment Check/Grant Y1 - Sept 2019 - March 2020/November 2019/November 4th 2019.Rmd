...
directory<-file.path("/Users/bbh5255/Box/DEPENd/NeuroMAP/Recruitment/Participant Management/") #set personal directory

#read in data for master file spreadsheet and R01 information - REMOVE PASSWORD FROM MASTER
MASTER <- read_xlsx("/Users/bbh5255/Desktop/NEUROMAP Master V2.xlsx", sheet = "MasterData")
ENROLL_OVERVIEW <- read_xlsx("/Users/bbh5255/Desktop/NEUROMAP Master V2.xlsx", sheet = "RO1 Aggregate", col_names = TRUE)
ENROLL_MASTER <- read_xlsx("/Users/bbh5255/Desktop/NEUROMAP Master V2.xlsx", sheet = "Tracker")
SCHEDULE <-read_csv("/Users/bbh5255/Box/DEPENd/NeuroMAP/Recruitment/Participant Management/Acuity Session Reports/schedule2019-11-04.csv") #read in acuity schedule info
...
## RO1 Enrollment Targets For November

```{r October RO1 Enrollment Overview, eval=FALSE, include=FALSE}
...
```{r November R01 Enrollment Overview, echo=FALSE}

#Totals for the month

NOV_RO1 <-ENROLL_OVERVIEW %>%
  select(YEAR, MONTH, GROUP, EXPECTED, PROJECTED, `ENROLLED FMRI`, `ENROLLED NO FMRI`, `TOTAL EXCLUDED`, C_EXPECTED) %>%
    filter(MONTH == 8 & YEAR == 1) 
...
knitr::kable(
  NOV_RO1, 
  caption = "November R01 Enrollment Targets",
  col.names = c("Diagnostic Group", "Expected Enrollment to Fulfill R01 Timeline in the Month of November", "In Process of Enrollment", "Enrolled in FMRI Group", "Not Enrolled in FMRI Group/Unassigned", "Excluded"))
```
...
knitr::kable(
  RO1_TOTAL,
  caption = "Cummulative R01 Enrollment To Date (Aug - November)",
...
#change date to be the date session is ran +7 days for start time
...
enrollment_pastweek_visual <-MASTER %>% 
  ...
  dplyr::filter((SCREEN_SENTDATE >= as.Date("2019-10-27") & SCREEN_SENTDATE <= as.Date("2019-11-04") & !is.na(STATUS))) %>%
  group_by(STATUS) %>%
  tally()%>%
  ggplot(aes(x=reorder(STATUS,-n), y=n, fill = STATUS)) +
    geom_bar(stat="identity") +
  scale_fill_brewer("Spectral") +
    ...
    coord_flip() + 
  theme(legend.position="none") +
  ggtitle("Activity from Past Week (October 27th - November 4th)")
...
source_contacts_pastweek_graph <- MASTER %>% 
  ...
  filter(SCREEN_SENTDATE >= as.Date("2019-10-27") & SCREEN_SENTDATE <= as.Date("2019-11-04")) %>%
  ggplot(aes(SOURCE)) +
