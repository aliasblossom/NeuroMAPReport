...
START_WEEK <- mdy("11/18/2019") #only change me, set as day of report
GY <- 1 #year of grant, change ?
RO1_TABLE_CAPTION <- "November R01 Enrollment Targets"
RO1_TOTAL_CAPTION <- "Cummulative R01 Enrollment To Date (Aug - November)"
...
MASTER <- read_xlsx(paste0(directory_home, "NEUROMAP Master V2.xlsx"), sheet = "MasterData")
ENROLL_OVERVIEW <- read_xlsx(paste0(directory_home, "NEUROMAP Master V2.xlsx"), sheet = "RO1 Aggregate", col_names = TRUE)
TRACKER <- read_xlsx(paste0(directory_home, "NEUROMAP Master V2.xlsx"), sheet = "Tracker")
...
enrollment_pastweek_visual <-MASTER %>% 
  ...
  scale_fill_brewer("Spectral") +
    ...
    coord_flip() + 
  theme(legend.position="none") +
  ggtitle("Activity from Past Week (November 11th - November 18th)")
...
TRACKER<-TRACKER %>%
  mutate(current_session=case_when(
    ...
    .$STATUS == "Completed S3" & .$`FMRI GROUP (Y/N)` == "Y" ~ "S4",
    .$STATUS == "Completed S3" & .$`FMRI GROUP (Y/N)` == "N" ~ "ENDT1NOFMIR",
