...
knitr::kable(
  ...
  col.names = c("Diagnostic Group", "Cumulative Expected Enrollment to Fulfill R01 Timeline", "In Process of Enrollment", "Enrolled in FMRI Group", "Not Enrolled in FMRI Group", "Excluded"))
...
UPCOMMING_ALL <- ENROLL_MASTER %>%
  ...
  filter(S1_DATE >= as.Date("2019-09-23") & S1_DATE <= as.Date("2019-10-30"))  %>%
  arrange(S1_DATE)

knitr::kable(
  UPCOMMING_ALL, 
  caption = "All Upcomming Session 1's",
...
UPCOMMING_WEEK <- ENROLL_MASTER %>%
  ...
  filter(S1_DATE >= as.Date("2019-09-23") & S1_DATE <= as.Date("2019-09-30"))
...
enrollment_pastweek_visual <-MASTER %>% 
  ...
  dplyr::filter((SCREEN_SENTDATE >= as.Date("2019-09-16") & SCREEN_SENTDATE <= as.Date("2019-09-23") & !is.na(STATUS))) %>%
  group_by(STATUS) %>%
  tally()%>%
  ggplot(aes(x=reorder(STATUS,-n), y=n, fill = STATUS)) +
    geom_bar(stat="identity") +
  scale_fill_brewer("Spectral") +
    ...
    coord_flip() + 
  theme(legend.position="none") +
  ggtitle("Activity from Past Week (Sep 16th - Sep 23rd)")
...
source_contacts_pastweek_graph <- MASTER %>% 
  ...
  filter(SCREEN_SENTDATE >= as.Date("2019-09-15") & SCREEN_SENTDATE <= as.Date("2019-09-23")) %>%
  ggplot(aes(SOURCE)) +
