...
START_WEEK <- mdy("12/16/2019") #only change me, set as day of report
...
ENROLL_PASTWEEK_TITLE<- "Activity from Past Week (December 9th - December 16th)" 
...
TRACKER<-TRACKER %>%
  mutate(current_session=case_when(
    ...
    .$STATUS == "Scheduled S3, Scheduled S4" ~ "S4",
...
#%>%
  ...
  t<-ggplot(data=TRACKER, aes(x=current_ses, y=len)) +
  geom_bar(stat="identity")
