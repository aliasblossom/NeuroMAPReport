...
START_WEEK <- mdy("02/14/2020") #only change me, set as date of neuromap Startup meeting - 7 days
...
ENROLL_PASTWEEK_TITLE<- "Activity from Past Week (Feb 13th - Feb 21st)" 
```
...
## RO1 Enrollment Targets For February
...
SCHEDULE %>% 
  select(
    ...
    `Type`,  
    `Calendar`) %>%
  dplyr::filter(`Start Time` >= END_WEEK & (`Type (Recode)` == "Session 1" | `Type (Recode)` == "Session 1 FU"))) %>%
