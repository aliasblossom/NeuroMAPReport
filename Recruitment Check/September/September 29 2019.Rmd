...
setwd("/Users/strength/Box Sync/DEPENd/NeuroMAP/Recruitment/Participant Management/")
...
ACUITY<-read_csv(paste0(directory, "acuity_export_2019-09-22.csv"))
colnames(ACUITY)  
...
knitr::kable(
  ...
  col.names = c("Diagnostic Group", "Expected Enrollment to Fulfill R01 Timeline in the Month of September", "In Process of Enrollment", "Enrolled in FMRI Group", "Not Enrolled in FMRI Group/Unassigned", "Excluded"))
...
TALLY_UPCOMING_SESSIONS <- SCHEDULE %>% select(`First Name`, `Last Name`, `Start Time`, `Type`, `Calendar`) %>%
  dplyr::filter(`Start Time` >= as.Date("2019-09-30") & `Start Time` <= as.Date("2019-10-07")) %>%
  group_by(`Type`) %>%
  tally() %>%
  arrange(desc(n))
...
dplyr::filter((`Start Time` >= as.Date("2019-09-30") & `Start Time` <= as.Date("2019-10-07")) & (`Type` == "Session 1"))
...
dplyr::filter((`Start Time` >= as.Date("2019-09-30") & `Start Time` <= as.Date("2019-10-07")) & (`Type` == "Session 1 Follow Up (1.5 hours)" | `Type` == "Session 1 Follow Up (30 minutes)" | `Type` == "Session 1 Follow Up (3 hours)"))
...
dplyr::filter((`Start Time` >= as.Date("2019-09-30") & `Start Time` <= as.Date("2019-10-07")) & (`Type` == "Session 2"))
...
dplyr::filter((`Start Time` >= as.Date("2019-09-30") & `Start Time` <= as.Date("2019-10-07")) & (`Type` == "Session 3"))
...
dplyr::filter(`Start Time` >= as.Date("2019-10-07") & `Start Time` <= as.Date("2019-10-14"))
# 
knitr::kable(
UPCOMMING_ALL, 
...
dplyr::filter(`Start Time` >= as.Date("2019-09-22") & `Start Time` <= as.Date("2019-09-29"))
...
s1 <- read_csv(paste0(directory,"Session 1 Check List/Session 1 Check List September 23 2019.csv"), col_names = TRUE) %>% 
  #, col_types = cols(
...
enrollment_pastweek_visual <-MASTER %>% 
  ...
  dplyr::filter((SCREEN_SENTDATE >= as.Date("2019-09-23") & SCREEN_SENTDATE <= as.Date("2019-09-29") & !is.na(STATUS))) %>%
  group_by(STATUS) %>%
  tally()%>%
  ggplot(aes(x=reorder(STATUS,-n), y=n, fill = STATUS)) +
    geom_bar(stat="identity") +
  scale_fill_brewer("Spectral") +
    ...
    coord_flip() + 
  theme(legend.position="none") +
  ggtitle("Activity from Past Week (Sep 23rd - Sep 29th)")
...
source_contacts_pastweek_graph <- MASTER %>% 
  ...
  filter(SCREEN_SENTDATE >= as.Date("2019-09-23") & SCREEN_SENTDATE <= as.Date("2019-09-29")) %>%
  ggplot(aes(SOURCE)) +
