...
ENROLL_OVERVIEW <- read_xlsx("/Users/strength/Box Sync/DEPENd/NeuroMAP/Recruitment/Participant Management/NEUROMAP Participant Tracker.xlsx", sheet = "RO1 Aggregate", col_names = TRUE)
ENROLL_MASTER <- read_xlsx("/Users/strength/Box Sync/DEPENd/NeuroMAP/Recruitment/Participant Management/NEUROMAP Participant Tracker.xlsx", sheet = "Tracker")
...
#clean up R01 enrollment overview
...
### RO1 Enrollment Target For Current Month
...
SEPT_RO1 <-ENROLL_OVERVIEW %>%
  select(YEAR_GRANT, MONTH, GROUP, EXPECTED, PROJECTED, `ENROLLED FMRI`, `ENROLLED NO FMRI`) %>%
    filter(MONTH == "Sep" & YEAR_GRANT == 1) 

SEPT_RO1 <-SEPT_RO1 %>%
  select(GROUP, EXPECTED, PROJECTED, `ENROLLED FMRI`, `ENROLLED NO FMRI`)

knitr::kable(
  SEPT_RO1, 
  caption = "September R01 Enrollment Targets",
  col.names = c("Diagnostic Group", "Expected Enrollment to Fulfill R01 Timeline", "In Process of Enrollment", "Enrolled in FMRI Group", "Not Enrolled in FMRI Group"))

#TOTAL TO DATE

RO1_TOTAL <-ENROLL_OVERVIEW %>%
  select(YEAR_GRANT, MONTH, GROUP, `C_ENROLLED FMRI`,`C_ENROLLED NO FMRI`,C_EXCLUDED,C_EXPECTED,C_PROJECTED ) %>%
    filter(MONTH == "Sep" & YEAR_GRANT == 1)
select(RO1_TOTAL, GROUP, `C_ENROLLED FMRI`,`C_ENROLLED NO FMRI`,C_EXCLUDED,C_EXPECTED,C_PROJECTED)

knitr::kable(
  RO1_TOTAL, 
  caption = "R01 Enrollment To Date",
  col.names = c("Grant Year", "Month", "Diagnostic Group", "Enrolled in FMRI Group", "Not Enrolled in FMRI Group", "Excluded", "Expected Enrollment for R01", "In Process of Enrollment"))
...
```{r eval=FALSE, include=FALSE}
TODATE_RO1 <-ENROLL_OVERVIEW %>%
  select(YEAR_GRANT, MONTH, GROUP, EXPECTED, PROJECTED, `ENROLLED FMRI`, `ENROLLED NO FMRI`) %>%
    filter(MONTH == "Aug" | MONTH == "Sep" & YEAR_GRANT == 1)
...
knitr::kable(
  SEPT_RO1, 
  caption = "September R01 Enrollment Targets",
  col.names = c("Grant Year", "Month", "Diagnostic Group", "Expected Enrollment to Fulfill R01 Timeline", "In Process of Enrollment", "Enrolled in FMRI Group", "Not Enrolled in FMRI Group"))

```


### Overview of September Enrollment

```{r eval=FALSE, include=FALSE}
MONTH_TOTAL <- ENROLL_OVERVIEW %>%
  select(GROUP, S1_DATE, R01_ENROLLMENT) %>%
  filter(S1_DATE >= as.Date("2019-09-01") & S1_DATE <= as.Date("2019-09-31")) %>%
  group_by(GROUP, R01_ENROLLMENT) %>%
  tally()
  
knitr::kable(
  MONTH_TOTAL, 
  caption = "Overview of September Enrollment by Group",
