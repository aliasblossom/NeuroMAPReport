...
MASTER <- read_xlsx("/Users/bbh5255/Box Sync/DEPENd/NeuroMAP/Recruitment/Participant Management/NEUROMAP Master V2.xlsx")
ENROLL_OVERVIEW <- read_xlsx("/Users/bbh5255/Box Sync/DEPENd/NeuroMAP/Recruitment/Participant Management/NEUROMAP Participant Tracker.xlsx", sheet = "RO1 Aggregate", col_names = TRUE)
ENROLL_MASTER <- read_xlsx("/Users/bbh5255/Box Sync/DEPENd/NeuroMAP/Recruitment/Participant Management/NEUROMAP Participant Tracker.xlsx", sheet = "Tracker")
...
SEPT_RO1 <-ENROLL_OVERVIEW %>%
  select(YEAR_GRANT, MONTH, GROUP, EXPECTED, PROJECTED, `ENROLLED FMRI`, `ENROLLED NO FMRI`, `TOTAL EXCLUDED`, C_EXPECTED) %>%
...
RO1_TOTAL <-ENROLL_OVERVIEW %>%
  select(YEAR_GRANT, MONTH, GROUP, C_EXPECTED, `C_ENROLLED FMRI`,`C_ENROLLED NO FMRI`,C_EXCLUDED,C_PROJECTED ) %>%
    filter(MONTH == "Sep" & YEAR_GRANT == 1)
...
knitr::kable(
  ...
  col.names = c("Diagnostic Group", "Expected Enrollment to Fulfill R01 Timeline To Date", "In Process of Enrollment", "Enrolled in FMRI Group", "Not Enrolled in FMRI Group", "Excluded"))
...
UPCOMMING_ALL <- ENROLL_MASTER %>%
  ...
  filter(S1_DATE >= as.Date("2019-09-09") & S1_DATE <= as.Date("2019-09-30")) %>%
  arrange(S1_DATE)
...
LASTWEEK <- ENROLL_MASTER %>%
  select(NAME, ID, AGE, GENDER, R01_ENROLLMENT, S1_DATE,'S1 FU 1') %>%
  filter(S1_DATE >= as.Date("2019-09-02") & S1_DATE <= as.Date("2019-09-09"))


knitr::kable(
  LASTWEEK, 
  caption = "Sessions Scheduled Last Week",
  col.names = c("Name", "ID", "Age", "Gender", "fMRI Elegible", "Date of Session 1", "Date of Session 1 - Follow-Up"))
...
s1<-read.csv(file = "/Users/bbh5255/Box Sync/DEPENd/NeuroMAP/Recruitment/Participant Management/S1CHECK.csv", header = TRUE) %>% #read in data for session 1 checklist 
  slice(-c(1:3)) %>%
