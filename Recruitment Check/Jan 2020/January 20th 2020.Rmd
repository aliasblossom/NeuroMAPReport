...
START_WEEK <- mdy("01/20/2020") #only change me, set as day of report
GY <- 1 #year of grant, change ?
RO1_TABLE_CAPTION <- "January R01 Enrollment Targets"
RO1_TOTAL_CAPTION <- "Cummulative R01 Enrollment To Date (Aug 2019 - Jan 2020)"
ENROLL_PASTWEEK_TITLE<- "Activity from Past Week (January 10th - January 16th)" 
```


```{r Import + Clean Data for Enrollment Master, include=FALSE}
#setwd("Box/DEPENd/NeuroMAP/Recruitment/Participant Management/")
directory_onedriveH<-file.path("/Users/strength/OneDrive - The Pennsylvania State University/NEUROMAP Master V2.xlsx/") #set personal directory
...
SCHEDULE <-read_csv(paste0(directory_work, "Acuity Session Reports/schedule2020-01-20 (1).csv")) #read in acuity schedule info
...
```{r Import Qualtrics Data, include=FALSE}
...
```{r Session Tally, eval=FALSE, include=FALSE}

#total number of appointments that are not canceled/no-show by person
SCHEDULE %>% select(Calendar, Canceled) %>%
  filter(is.na(Canceled)) %>%
  group_by(Calendar) %>%
  tally()

SCHEDULE %>% select(Calendar, Canceled, Type) %>%
  filter(is.na(Canceled)) %>%
...
S3_Report %>% 
  ...
  drop_na()%>%
  mutate(
    `s3overall_score_1` = cell_spec(`s3overall_score_1`, "html", color = ifelse(s3overall_score_1 > 3, "green", "red"))) %>%
...
S3_Report %>% 
  ...
  rename(
    ...
    ID = s3_sessioninfo_4_TEXT,
    `Experimenter` = s3_sessioninfo_3_TEXT,
  ...
  column_spec(1, bold = T, width = "1") %>%
  column_spec(2, bold = F, width = "1") %>%
  column_spec(3, bold = F, border_right = T, width = "1") %>%
  column_spec(4, width = "2")
```
...
knitr::kable(
  ...
  col.names = c("Diagnostic Group", "Expected Enrollment to Fulfill R01 Timeline in the Month of January", "In Process of Enrollment", "Enrolled in FMRI Group", "Not Enrolled in FMRI Group", "Status TBD", "Excluded"))
```
...
```{r Session 3 For Current Week, echo=FALSE}
...
```{r Current Week Session 4, echo=FALSE}
...
TRACKER<-TRACKER %>%
  ...
  t<-ggplot(data=TRACKER, aes(x=current_session)) +
  geom_bar(stat="count")
