...
directory<-file.path("/Users/strength/Box/DEPENd/NeuroMAP/Recruitment/Participant Management/") #set personal directory

#read in data for master file spreadsheet and R01 information - REMOVE PASSWORD FROM MASTER
MASTER <- read_xlsx(paste0(directory, "Copy of NEUROMAP Master V2.xlsx"), sheet = "MasterData")
ENROLL_OVERVIEW <- read_xlsx(paste0(directory, "Copy of NEUROMAP Master V2.xlsx"), sheet = "RO1 Aggregate", col_names = TRUE)
ENROLL_MASTER <- read_xlsx(paste0(directory, "Copy of NEUROMAP Master V2.xlsx"), sheet = "Tracker")
...
enrollment_pastweek_visual <-MASTER %>% 
  ...
  dplyr::filter((SCREEN_SENTDATE >= as.Date("2019-11-04") & SCREEN_SENTDATE <= as.Date("2019-11-11") & !is.na(STATUS))) %>%
  group_by(STATUS) %>%
  tally()%>%
  ggplot(aes(x=reorder(STATUS,-n), y=n, fill = STATUS)) +
    geom_bar(stat="identity") +
  scale_fill_brewer("Spectral") +
    ...
    coord_flip() + 
  theme(legend.position="none") +
  ggtitle("Activity from Past Week (November 4th - November 11th)")
...
source_contacts_pastweek_graph <- MASTER %>% 
  ...
  filter(SCREEN_SENTDATE >= as.Date("2019-11-04") & SCREEN_SENTDATE <= as.Date("2019-11-11")) %>%
  ggplot(aes(SOURCE)) +
...
#ENROLL_MASTER[,13] <- ifelse(ENROLL_MASTER[,13] == "M", 1, ifelse(A[,1] == "F", 2, 99))
