...
p_load("ggplot2", "dplyr", "tidyr", "cowplot", "qualtRics", "readxl", "tidyverse", "lubridate", "RColorBrewer", "kableExtra", "dygraphs", "xts", "ggtext", "DT")
...
output <- ws_path %>% excel_sheets() %>% set_names() %>%
  map_dfr(read_excel(col_types = "numeric"), path = ws_path)
...
#EnrollOverview <- load_pdata(account = "strength", workbook = "Enroll Overview")
...
```{r Qualtrics API Credentials, include=FALSE}
...
```{r Session 4 Report Data Cleaning, include=FALSE}
S4_Report<-S4_Report %>%
...
datatable(s3_shrooms)
```

```{r Session 3 Report Ratings, eval=FALSE, include=FALSE}

#decpreciate code block
S3_Report %>% 
  select("S3Date",
         "ID",
         "Researcher",
         "Overall Rating",
         "Vanilla Baseline Rating (Physio)",
         "Vanilla Baseline Rating (EyeTraking)",
         "Vanilla Baseline Rating (Behavior)",
         "DTK Rating (Physio)",
         "DTK Rating (EyeTraking)",
         "DTK Rating (Behavior)",
         "Mushrooms Rating (Physio)",
         "Mushrooms Rating (EyeTraking)",
         "Mushrooms Rating (Behavior)",
         "Neighborhood Rating (Physio)" ,
         "Neighborhood Rating (EyeTraking)" ,
         "Neighborhood Rating (Behavior)",
         "Vending Machine Rating (Physio)",
         "Vending Machine Rating (EyeTraking)" ,
         "Vending Machine Rating (Behavior)") %>%
  dplyr::filter((.$S3Date>= params$report_start & .$S3Date <= params$report_end)) %>%
  arrange(S3Date) %>%
 # mutate(
  #  `Overall Rating` = cell_spec (`Overall Rating`, "html", 
   #                               color = if_else (`Overall Rating` > 3, "green",
    #                              if_else(`Overall Rating` == "NA", "blue", "red")))) %>%
  kable(format = "html", escape = F, caption = "Session 3 Ratings",
         col.names = c("Date", "ID", "RA", "Overall", "P", "ET", "B", "P", "ET", "B","P", "ET", "B","P", "ET", "B", "P", "ET", "B")) %>%
  kable_styling(bootstrap_options = "condensed", full_width = F) %>%
  add_header_above(c(" " = 4, "Vanilla Baseline" = 3, "DTK" = 3, "Mushrooms" = 3, "Neighborhood" = 3, "Vending Machine" = 3)) %>%
  footnote(
...
#factor each level so they can be reordered in the natural format to tell a better story
#NA is naturally factored
Master$STATUS <- factor(Master$STATUS, levels = c("No Longer Interested", "Inquiry", "Screen Sent", "Completed Screen", "Invited S1", "Queued S1"))

#reverse the factor levels (see https://stackoverflow.com/questions/42710056/reverse-stacked-bar-order)
Master$STATUS <-fct_rev(Master$STATUS)

#create a plot based on this data
Master %>%
  drop_na(STATUS) %>%
  ggplot(aes(y = STATUS)) +
  geom_bar() +
  labs(
    title = "We have invited over half of the participants who completed the screen to enroll in the study") +
