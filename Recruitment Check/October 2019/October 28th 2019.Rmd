...
MASTER <- read_xlsx("NEUROMAP Master V2.xlsx", sheet = "MasterData")
ENROLL_OVERVIEW <- read_xlsx("NEUROMAP Master V2.xlsx", sheet = "RO1 Aggregate", col_names = TRUE)
ENROLL_MASTER <- read_xlsx("NEUROMAP Master V2.xlsx", sheet = "Tracker")
SCHEDULE <-read_csv("schedule2019-10-28.csv") #read in acuity schedule info
...
dplyr::filter((`Start Time` >= as.Date("2019-10-28") & `Start Time` <= as.Date("2019-11-04")) & (`Type` == "Session 1"))
...
dplyr::filter((`Start Time` >= as.Date("2019-10-28") & `Start Time` <= as.Date("2019-11-04")) & (`Type` == "Session 1 Follow Up (1.5 hours)" | `Type` == "Session 1 Follow Up (30 minutes)" | `Type` == "Session 1 Follow Up (3 hours)"))
...
dplyr::filter((`Start Time` >= as.Date("2019-10-28") & `Start Time` <= as.Date("2019-11-04")) & (`Type` == "Session 2"))
...
dplyr::filter((`Start Time` >= as.Date("2019-10-28") & `Start Time` <= as.Date("2019-11-04")) & (`Type` == "Session 3"))
...
dplyr::filter(`Start Time` >= as.Date("2019-11-04") & `Start Time` <= as.Date("2019-11-11"))
# 
knitr::kable(
UPCOMMING_ALL, 
...
dplyr::filter(`Start Time` >= as.Date("2019-10-21") & `Start Time` <= as.Date("2019-10-27"))
...
enrollment_pastweek_visual <-MASTER %>% 
  ...
  dplyr::filter((SCREEN_SENTDATE >= as.Date("2019-10-21") & SCREEN_SENTDATE <= as.Date("2019-10-27") & !is.na(STATUS))) %>%
  group_by(STATUS) %>%
  tally()%>%
  ggplot(aes(x=reorder(STATUS,-n), y=n, fill = STATUS)) +
    geom_bar(stat="identity") +
  scale_fill_brewer("Spectral") +
    ...
    coord_flip() + 
  theme(legend.position="none") +
  ggtitle("Activity from Past Week (October 21st - October 27th)")
...
source_contacts_pastweek_graph <- MASTER %>% 
  ...
  filter(SCREEN_SENTDATE >= as.Date("2019-10-21") & SCREEN_SENTDATE <= as.Date("2019-10-27")) %>%
  ggplot(aes(SOURCE)) +
